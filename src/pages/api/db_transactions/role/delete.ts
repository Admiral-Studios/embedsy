  ...
  if (req.method === 'POST') {
    try {
      ...
      if (id) {
        ...
        const updateUserRolesQuery = `UPDATE user_roles SET role_id = @guestRoleId WHERE role_id = @roleId`
        await ExecuteQuery(updateUserRolesQuery, { guestRoleId, roleId: id })
        ...
        await ExecuteQuery(query, { roleId: id })
