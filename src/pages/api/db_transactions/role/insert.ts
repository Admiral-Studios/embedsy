...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    if (role) {
      const checkRoleQuery = `SELECT COUNT(*) as count FROM roles WHERE role = @role`
      try {
        const roleExists = await ExecuteQuery(checkRoleQuery, { role })
        ...
        const query = `INSERT INTO roles (role, can_refresh, can_export, can_manage_own_account) 
                       VALUES (@role, @canRefresh, @canExport, @canManage); 
        ...
        res.status(500).json({ message: 'Failed to check role existence' })
      }
    } else {
