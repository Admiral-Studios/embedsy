...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  const suspendUrl = `https://management.azure.com/subscriptions/${capacityDetails.capacity_subscription_id}/resourceGroups/${capacityDetails.capacity_resource_group_name}/providers/Microsoft.${capacityDetails.capacity_type}/capacities/${capacityDetails.capacity_name}/suspend?api-version=${capacityApiVersion}`
  ...
  try {
    ...
    return response.status(200).json({ message: 'Capacity suspended successfully' })
  } catch (error: any) {
    return response.status(error.response?.status || 500).json({ error: 'Failed to suspend capacity' })
  }
}
