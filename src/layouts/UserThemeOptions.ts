...
import darkenColor from 'src/utils/darkenColor'
import hexToRgb from 'src/utils/hexToRgb'
import isColorDark from 'src/utils/isColorDark'

const UserThemeOptions = (appBranding: AppBranding | null): ThemeOptions => {
  // ** To use mode (light/dark/semi-dark), skin(default/bordered), direction(ltr/rtl), etc. for conditional styles, uncomment below line
  ...
  const mainColorLight = process.env.NEXT_PUBLIC_MAIN_COLOR_LIGHTER || '#FFE22F'
  const mainColor = appBranding?.main_color || process.env.NEXT_PUBLIC_MAIN_COLOR || '#FFC815'
  const mainColorDark = appBranding?.main_color_on_dark || process.env.NEXT_PUBLIC_MAIN_COLOR_DARK || '#E6AF00'
  const primaryMainColor = mode === 'dark' ? mainColorDark : mainColor
  const primaryHoverColor = darkenColor(primaryMainColor, -15)

  const [r, g, b] = hexToRgb(primaryMainColor)
  const isPrimaryMainColorDark = isColorDark(r, g, b)
  ...
  return {
    palette: {
      ...
      customColors: {
        contrastTextColor: isPrimaryMainColorDark ? whiteColor : blackColor
      }
    },
    components: {
      MuiButton: {
